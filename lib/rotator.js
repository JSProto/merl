...
let minimumWorkTime = 5 * 60;                  // minutes
...
Rotator.init = _.throttle(co.wrap(function* (){
    ...
    let grouped = _.chain(Rotator.db.vms)
        ...
        .cloneDeep()
        .map(function(vm){
            let today_time_seconds = moment.duration(vm.today_time).as('seconds');
            vm.today_time_seconds = today_time_seconds;
            return vm;
        })
        .groupBy('host')
        .value();
...
class MachineSchedule {
    ...
    * startMachine () {
        let vm = this.getVM();
        let stopDelay = workTime;

        if (vm.merl_access) {
            stopDelay = workTime - vm.today_time_seconds;

            if (stopDelay < 0) {
                stopDelay = workTime;
            }
            else {
                stopDelay = stopDelay < minimumWorkTime ? minimumWorkTime : stopDelay;
            }
        }
        ...
        if (state != 'Running') {
            ...
            this.addScheduleStopAfter(stopDelay);
        }
        else {
            Rotator.init();
        }
